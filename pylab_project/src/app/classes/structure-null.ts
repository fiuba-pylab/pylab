...
export class NullStructure extends Structure {
  ...
  override async execute(): Promise<{ amount: number; finish: boolean }> {
    ...
    const variableDeclaration = this.lines[0].match(
      REGEX_CONSTS.REGEX_VARIABLE_DECLARATION
    );
    const operations = this.lines[0].match(REGEX_CONSTS.REGEX_OPERATIONS);
    const collectionAdd = this.lines[0].match(
      REGEX_CONSTS.REGEX_COLLECTION_ADD
    );
    const collectionSubstract = this.lines[0].match(
      REGEX_CONSTS.REGEX_COLLECTION_SUBSTRACT
    );
    ...
    if (variableDeclaration) {
      ...
      let varValue = await this.applyFunctions(
        variableDeclaration[2],
        variables,
        varName
      );
      let collection = await this.matchCollection(
        varValue,
        variables,
        variableDeclaration[2]
      );
    ...
    if (operations) {
      ...
      variables[variable].push(
        this.applyOperation(
          Number(replaceVariables(variable, variables)),
          operator,
          Number(replaceVariables(value, variables))
        )
      );
    }
    if (print) {
    ...
    if (collectionAdd) {
      ...
      if (VALID_OPERATORS.validAddOperators.includes(operator)) {
        variables[variable].add(
          await this.applyFunctions(value, variables, variable)
        );
  ...
  async matchCollection(
    ...
    //se accede a una colecion
      ...
      (collectionAccess = collectionName.match(
        REGEX_CONSTS.REGEX_COLLECTION_ACCESS
      ))
    ) {
  ...
  async evaluateExpression(
    ...
    do {
      ...
      currentExpression = await this.replaceAsync(
        currentExpression,
        REGEX_CONSTS.REGEX_FUNCTIONS,
        async (match, funcName, args) => {
          let evaluatedArgs = await Promise.all(
            args
              .split(',')
              .map(
                async (arg: string) =>
                  await this.evaluateExpression(arg.trim(), varName)
              )
          );
